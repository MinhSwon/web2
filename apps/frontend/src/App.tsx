...
function AuthScreen({ onAuthenticated }: { onAuthenticated: (token: string, user: User) => void }) {
  ...
  useEffect(() => {
    ...
    if (g?.accounts?.id) {
      try {
        ...
        if (container) {
          g.accounts.id.renderButton(container, { theme: "outline", size: "large", text: "continue_with" });
        }
      } catch (e) {
  ...
  async function googleLogin() {
    const email = prompt("Nhập địa chỉ Gmail của bạn để đăng nhập nhanh:");
    ...
    <main className="auth-page">
      ...
      <section className="auth-panel">
        <form onSubmit={submit} className="auth-form">
          ...
          <div id="google-gsi-btn" style={{ marginBottom: "12px", display: "flex", justifyContent: "center" }}></div>
